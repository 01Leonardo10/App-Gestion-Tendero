...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    if (isset($_GET['id_DocumentoPersona']) && isset($_GET['rep_LicenciaEstado'])) {
        $c_id_DocumentoPersona = $_GET['id_DocumentoPersona'];
        $c_rep_LicenciaEstado = $_GET['rep_LicenciaEstado'];
        
        if ($stmt = $conexion->prepare("CALL sp_m_estadolicencia_repartidor(?, ?)")) {
            $stmt->bind_param('ss', $c_id_DocumentoPersona, $c_rep_LicenciaEstado);
            
            if ($stmt->execute()) {
                $stmt->store_result();
                
                if ($stmt->affected_rows > 0) {
                    echo json_encode(['success' => 'Estado de licencia actualizado correctamente.']);
                } else {
